...
English：[https://bitcoin.org/bitcoin.pdf](https://bitcoin.org/bitcoin.pdf)
中文：[http://lixiaolai.com/Bitcoin-Whitepaper-EN-CN.html](http://lixiaolai.com/Bitcoin-Whitepaper-EN-CN.html)(译文)
...
>关键词句：将提出一种针对双重支出的解决方案，使用点对点的、分布式的时间戳服务器去生成基于算力的证明，按照时间顺序记录每条交易。
...
我们需要一种方式，可以让收款人确认之前的所有者并没有在任何之前的交易上签名。就我们的目的而言，只有最早的交易是算数的，所以，`我们并不关心其后的双重支付企图。确认一笔交易不存在的唯一方法是获悉所有的交易。`在铸币厂模型之中，铸币厂已然知悉所有的交易，并且能够确认这些交易的顺序。为了能在没有“被信任的一方”参与的情况下完成以上任务，`交易记录必须被公开宣布`，进而我们需要一个`系统能让参与者们认同它们所接收到的同一个唯一的交易历史。收款人需要证明在每笔交易发生之时，大多数节点能够认同它是第一个被接收的`。

>关键词句：这一段是提出了数字签名链的方法，但还有一个问题，确认一笔交易不存在的唯一方法是获悉所有的交易，如何在没有被信任的一方参与的情况下，让参与者们认同它们所接收到的都是同一个且唯一的交易历史。
...
> 关键词：逐步提出解决问题`让参与者们认同它们所接收到的都是同一个且唯一的交易历史`的方案，首先工作证明就是去寻找一个数值，这个散列数值必须以一定数量的 0 开头。
...
> 关键词句: 引入随机数，因为计算量巨大，如果想改变链上某一个块，这个块之后所有的块都要重新计算才能满足条件，因此几乎是不可能篡改。
...
> 关键词：为了应对硬件算力综合的不断增加，提出了动态改变工作证明难度的方法。
...
>关键词句：最长链是正确的那个，如果多个节点都算出来了那个数值，就就看下一个工作证明谁能能成为更长的链。
...
## part 7 - 回收硬盘空间 (Reclaiming Disk Space)
...
>关键词句：为了节省磁盘空间，引入梅克尔树。这一段原理我也不太懂，后面再更新。

## part 7 -  简化版支付确认 (Simplified Payment Verification)
...
>关键词句：简化支付确认 - 即可以不用保存全部的交易记录（全节点）只通过最长链的区块头拷贝，获取梅克尔树的树枝节点，然后检查交易。

## part 8 - 价值的组合与分割 (Combining and Splitting Value)
...
值得注意的是，“fan-out”在这里并不是问题 —— 所谓“fan-out”，就是指一笔交易依赖于数笔交易，且这些交易又依赖于更多笔交易。从来就没有必要去提取任何一笔交易的完整独立的历史拷贝。

>关键词句：UTXO，比特币是通过输入输出来记录交易的。这个地方其实不太理解。
...
>关键词句：因为交易是全公开的，那么如何保障隐私呢？答案就是公钥私钥。

## part 10 - 计算 (Calculations)
这一段有很多数学公式复制不过来。
>关键词句：用数学的方式证明了之前提出的方案是可靠的。
...
>总结：比特币可以说是结合了密码学和博弈论，点对点（p2p）的分布式系统如何保持诚信，这也是通常所说的拜占庭将军（PBFT）问题，有兴趣的可以搜一搜。